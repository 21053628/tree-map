...
function cacheWithTimestamp(cache,req,res){
  if(res.type==='opaque'||res.status===0) return cache.put(req,res).catch(function(){ /* intentionally ignored: optional fallback failure */ });
  var headers=new Headers(res.headers); headers.set('x-sw-cached-at',String(Date.now()));
  return res.blob().then(function(blob){ var nr=new Response(blob,{status:res.status,statusText:res.statusText,headers:headers}); return cache.put(req,nr); }).catch(function(){ /* intentionally ignored: optional fallback failure */ });
}
...
function getSwMaxAgeForRequest(req){
  try{ var u=new URL(req.url); var act=(u.searchParams.get('action')||'').toLowerCase();
    ...
    if(CachePolicy.getSwMaxAge) return CachePolicy.getSwMaxAge(act);
  }catch(e){ /* intentionally ignored: optional fallback failure */ } return 600000;
}
...
function checkQuotaAndShrink(){
  try{ if(navigator.storage&&navigator.storage.estimate){
    navigator.storage.estimate().then(function(est){
      ...
      if(est.usage/est.quota>0.8){ trimCacheLRU(TILE_CACHE,Math.floor(TILE_MAX*0.5),TILE_MAX_AGE).catch(function(){ /* intentionally ignored: optional fallback failure */ }); trimCacheLRU(IMG_CACHE,Math.floor(IMG_MAX*0.5),IMG_MAX_AGE).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }
    }).catch(function(){ /* intentionally ignored: optional fallback failure */ });
...
function handleInvalidateDataCache(type,payload){
  ...
  return caches.open(DATA_CACHE).then(function(c){ return c.keys().then(function(keys){
    var dels=keys.filter(function(req){
      ...
      if(pid&&url.indexOf('action=trees')!==-1){ try{ var u=new URL(url); var p=u.searchParams.get('project')||u.searchParams.get('prj')||''; if(p&&p!==pid) return false; }catch(e){ /* intentionally ignored: optional fallback failure */ } }
      return true;
...
self.addEventListener('activate',function(e){
  e.waitUntil((async function(){
    if('navigationPreload' in self.registration){ try{ await self.registration.navigationPreload.enable(); }catch(err){ /* intentionally ignored: optional fallback failure */ } }
...
self.addEventListener('message',function(e){
  ...
  if(e.data&&e.data.type==='REGISTER_BG_SYNC'){ if('sync' in self.registration) self.registration.sync.register('sync-outbox').catch(function(){ /* intentionally ignored: optional fallback failure */ }); return; }
...
function handleNavigation(req){
  return fetchWithAbort(req,3500).then(function(res){
    if(res.ok){ var c=res.clone(); caches.open(PRECACHE_NAME).then(function(cache){ cacheWithTimestamp(cache,req,c); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }
    return res;
  }).catch(function(){
    return caches.match(req).then(function(cached){
      if(cached) return cached;
      try{ var url=new URL(req.url); var p=url.pathname; if(p.indexOf('/t.html')!==-1) return caches.match('./t.html'); if(p.indexOf('/nfc.html')!==-1) return caches.match('./nfc.html'); if(p.indexOf('/offline.html')!==-1) return caches.match('./offline.html'); }catch(e){ /* intentionally ignored: optional fallback failure */ }
...
function handleApi(req){
  ...
  var isBypass=false; try{ var u=new URL(req.url); isBypass = u.searchParams.get('nocache')==='1' || u.searchParams.get('bust')==='1'; }catch(e){ /* intentionally ignored: optional fallback failure */ }
  ...
  if(isBypass){
    return fetchWithAbort(req,API_TIMEOUT).then(function(res){
      var ct=''; try{ ct=res.headers.get('content-type')||''; }catch(e){ /* intentionally ignored: optional fallback failure */ }
      if(ct.indexOf('text/html')!==-1){ console.warn('[SW] API_HTML_RESPONSE bypass, not caching '+req.url); return res; }
      if(res.ok){ var c=res.clone(); caches.open(DATA_CACHE).then(function(cache){ cacheWithTimestamp(cache,req,c).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }
      return res;
    }).catch(function(){
  ...
  return fetchWithAbort(req,API_TIMEOUT).then(function(res){
    ...
    if(res.ok){ var c=res.clone(); caches.open(DATA_CACHE).then(function(cache){ cacheWithTimestamp(cache,req,c).then(function(){ return trimCacheLRU(DATA_CACHE,DATA_MAX_ENTRIES,null); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); checkQuotaAndShrink(); }
    return res;
  }).catch(function(){
    return caches.match(req).then(function(cached){
...
function handleTiles(req){
  return caches.match(req).then(function(cached){
    ...
    if(cached){ fetch(req).then(function(res){ if(res.ok){ var c=res.clone(); caches.open(TILE_CACHE).then(function(cache){ cacheWithTimestamp(cache,req,c).then(function(){ return trimCacheLRU(TILE_CACHE,TILE_MAX,TILE_MAX_AGE); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); } }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); return cached; }
    return fetch(req).then(function(res){ if(res.ok){ var c=res.clone(); caches.open(TILE_CACHE).then(function(cache){ cacheWithTimestamp(cache,req,c).then(function(){ return trimCacheLRU(TILE_CACHE,TILE_MAX,TILE_MAX_AGE); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); } return res; });
  });
}
function handleImages(req){
  ...
  return caches.match(nUrl).then(function(cached){
    ...
    if(cached){ fetch(req).then(function(res){ if(res.ok){ var c=res.clone(); caches.open(IMG_CACHE).then(function(cache){ cacheWithTimestamp(cache,nUrl,c).then(function(){ return trimCacheLRU(IMG_CACHE,IMG_MAX,IMG_MAX_AGE); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); } }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); return cached; }
    return caches.match(req).then(function(c2){
      if(c2&&isCacheFresh(c2,IMG_MAX_AGE)) return c2;
      return fetch(req).then(function(res){ if(res.ok){ var c=res.clone(); caches.open(IMG_CACHE).then(function(cache){ cacheWithTimestamp(cache,nUrl,c).then(function(){ return trimCacheLRU(IMG_CACHE,IMG_MAX,IMG_MAX_AGE); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); checkQuotaAndShrink(); } return res; });
    });
  });
}
  ...
  return caches.match(req).then(function(cached){
    var fp=fetch(req).then(function(res){
      if(res.ok||res.type==='opaque'){ var c=res.clone(); caches.open(RUNTIME_CACHE).then(function(cache){
        caches.match(req,{cacheName:PRECACHE_NAME}).then(function(inPre){ if(inPre) return; cacheWithTimestamp(cache,req,c).then(function(){ return trimCacheLRU(RUNTIME_CACHE,RUNTIME_MAX,RUNTIME_MAX_AGE); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }).catch(function(){ cacheWithTimestamp(cache,req,c).catch(function(){ /* intentionally ignored: optional fallback failure */ }); });
      }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }
      return res;
    }).catch(function(){
...
function handleEnvConfig(req){
  return fetchWithAbort(req,5000).then(function(res){ if(res.ok){ var c=res.clone(); caches.open(RUNTIME_CACHE).then(function(cache){ cacheWithTimestamp(cache,req,c); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); } return res; }).catch(function(){ return caches.match(req).then(function(c){ return c||Response.error(); }); });
}
self.addEventListener('fetch',function(e){
  ...
  if(req.mode==='navigate'){
    e.respondWith((e.preloadResponse?Promise.resolve(e.preloadResponse):Promise.resolve(null)).then(function(pre){
      if(pre){ var c=pre.clone(); caches.open(PRECACHE_NAME).then(function(cache){ cacheWithTimestamp(cache,req,c); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); return pre; }
