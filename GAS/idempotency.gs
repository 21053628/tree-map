/**
 * 🔥 快速防重檢查（ScriptCache 層面，避免鎖外相片上傳造成孤兒檔案）
 * 喺上傳相片前先 check 一次，如果 client_id 最近見過就 skip upload。
 * 由 doPost 喺 upload 相片前調用；由 checkDuplicate_ / markDuplicateFast_ 更新。
 */
function checkDuplicateFast_(clientId) {
  if (!clientId) return false;
  try {
    const cache = CacheService.getScriptCache();
    return cache.get('FAST_DUP_' + clientId) === '1';
  } catch(e) { return false; }
}

function markDuplicateFast_(clientId, ttl) {
  if (!clientId) return;
  try {
    const cache = CacheService.getScriptCache();
    cache.put('FAST_DUP_' + clientId, '1', ttl || 600);
  } catch(e) {}
}
