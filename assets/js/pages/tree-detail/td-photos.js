...
export function initPhotoPreview() {
  ...
  // 限制選擇器僅顯示允許格式
  try { fileInput.accept = limits.accept; } catch(e) { /* intentionally ignored: optional fallback failure */ }

  // 動態表單重新建立時，避免同一個 input 被重複綁定 change 事件。
  if (fileInput.dataset.photoPreviewBound === 'true') {
    try { fileInput.accept = limits.accept; } catch(e2) { /* intentionally ignored: optional fallback failure */ }
    return;
  }
  ...
  fileInput.addEventListener('change', function (e) {
    ...
    const cur = (TD && Array.isArray(TD.selectedPhotos)) ? TD.selectedPhotos.length : 0;
    ...
    for (let j=0;j<valid.length;j++) TD.selectedPhotos.push(valid[j]);
...
export function updatePhotoPreview() {
  ...
  photoCount.textContent = TD.selectedPhotos.length + ' / ' + lim.maxCount;
  // 達上限時禁用選擇器（UX 提示）
  const fileInput2 = $('#photo');
  if (fileInput2) {
    const atLimit = TD.selectedPhotos.length >= lim.maxCount;
  ...
  TD.selectedPhotos.forEach(function (file, index) {
...
export function removePhoto(index) {
  TD.selectedPhotos.splice(index, 1);
  updatePhotoPreview();
}
