...
export async function staffMode(){
  ...
  staffInitPromise = (async function(){
    ...
    if (checkinBtn && globalThis.checkin) checkinBtn.addEventListener('click', function(){ globalThis.checkin(); });
    const submitBtn = document.getElementById('submitInspectionBtn');
    if (submitBtn && globalThis.submitInspection) submitBtn.addEventListener('click', function(){ globalThis.submitInspection(); });
    const saveBtn = document.getElementById('saveTreeInfoBtn');
    if (saveBtn && globalThis.saveTreeInfo) saveBtn.addEventListener('click', function(){ globalThis.saveTreeInfo(); });
    ...
    if(!globalThis.allTreesLoaded){
      fetch('data/trees_data.json').then(function(r){ return r.json(); }).then(function(trees){
        ...
        globalThis.allTreesLoaded=true;
