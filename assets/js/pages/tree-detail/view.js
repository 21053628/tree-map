...
export async function render(t) {
  ...
  if (backBtn && globalThis.goBackToMap) backBtn.addEventListener('click', function (e) { globalThis.goBackToMap(e); });
  const goNfcBtn = document.getElementById('goNfcBtn');
  if (goNfcBtn && globalThis.goNFC) goNfcBtn.addEventListener('click', globalThis.goNFC);
  if (!globalThis._tPageDelegated) {
    globalThis._tPageDelegated = true;
    document.addEventListener('keydown', function (e) { if (e.key !== 'Escape') return; var modal = document.getElementById('imgModal'); if (modal && modal.classList.contains('show') && globalThis.closeZoom) globalThis.closeZoom(); });
    $('#app').addEventListener('click', function (e) {
      const zoomImg = (e.target && e.target.closest) ? e.target.closest('.zoomable-img') : null;
      if (zoomImg) { e.stopPropagation(); if (globalThis.zoomImage) globalThis.zoomImage(zoomImg.src); return; }
      const modal = document.getElementById('imgModal');
      if (modal && modal.classList.contains('show')) { if (e.target === modal || (e.target.classList && e.target.classList.contains('modal-close'))) { if (globalThis.closeZoom) globalThis.closeZoom(); } }
    });
  }
...
export function renderError(message) { const app = document.querySelector('#app'); if (app) app.innerHTML = message; }
