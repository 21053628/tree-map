...
function buildLogHtml(r) {
  ...
  if (photos.length > 0) {
    ...
    const treeIdEscaped = escapeHtml(r.tree_id || globalThis.TD.id);
    const A = '&';
...
export function attachLogsDelegation() {
  ...
  logs.addEventListener('click', function (e) {
    ...
    if (img) { e.stopPropagation(); const src = img.getAttribute('data-zoom'); if (src && globalThis.zoomImage) globalThis.zoomImage(src); return; }
    const btn = (e.target && e.target.closest) ? e.target.closest('.inspection-photo-btn') : null;
    if (btn) { e.stopPropagation(); const url = btn.getAttribute('data-download'); const tree = btn.getAttribute('data-tree'); const time = btn.getAttribute('data-time'); const idx = parseInt(btn.getAttribute('data-index') || '1', 10); if (globalThis.downloadPhoto) globalThis.downloadPhoto(url, tree, time, idx); }
  });
...
export function loadLogs(opts) {
  ...
  const tid = String(globalThis.TD.id || '');
  const prj = String(globalThis.TD.prj || '');
