...
import { TreeSnapshot, warmGAS } from '../../../offline.js';
...
function reconcileFromBootstrap_(bt, opts){
  ...
  for(var j=0;j<state.PROJECTS.length;j++){
    ...
    // 舊的保守合併會保留 existing 有而 incoming 冇嘅樹木，
  ...
  if(changed){
    if(state.isLocating){
      try{console.warn('[loader] reconcile deferred isLocating',changed);}catch(e){ /* intentionally ignored: optional fallback failure */ }
      setTimeout(function(){ try{ drawProjects(); drawTrees(); }catch(e){ /* intentionally ignored: optional fallback failure */ } }, 2600);
    } else {
...
export function applyTreesForProject(pid, trees, opts){
  ...
  if (!appendViewport) {
    ...
    if (pidCount > MAX_VIEWPORT_TREES) {
      ...
      // 非同步重載，唔阻礙當前 UI 更新
      setTimeout(function(){ loadTreesForProject(pid, { nocache: '1' }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }, 0);
    }
  }
  rebuildAllIndexes();
  ...
  if (opts.saveSnapshot !== false && TreeSnapshot && !appendViewport) {
    if (incoming.length>0 || opts.authoritative === true) {
      TreeSnapshot.save(snapKeyForProject(pid), incoming).catch(function(){ /* intentionally ignored: optional fallback failure */ });
    }
  }
  ...
  const p = (async function(){
    ...
    try{
      ...
      if (!opts.appendViewport && (!trees || trees.length===0)) {
        try {
          ...
          if (Array.isArray(bt) && bt.length) {
            ...
            if (filt.length) { try{ console.warn('[loader] bootstrap fallback hit '+filt.length+' for '+pid); }catch(e){ /* intentionally ignored: optional fallback failure */ } trees = filt; }
          }
        } catch(_e) { /* intentionally ignored: optional fallback failure */ }
      }
      ...
      if (!opts.appendViewport && TreeSnapshot) {
        ...
        }catch(_){ /* intentionally ignored: optional fallback failure */ }
      }
...
export async function loadProjects(opts){
  ...
  for (let attempt=0; attempt<3; attempt++){
    try{
      ...
      if (TreeSnapshot) TreeSnapshot.save(SNAP_PROJECTS, projects).catch(function(){ /* intentionally ignored: optional fallback failure */ });
      return projects;
    }catch(e){
      lastErr = e;
...
async function migrateLegacyMain(){
  ...
  try{
    ...
    for (const [pid, arr] of byPid){ await TreeSnapshot.save(snapKeyForProject(pid), arr).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }
  }catch(e){ /* intentionally ignored: optional fallback failure */ }
}
export async function load(){
  updateStatus('🗺️ 載入中…');
  // 🔥 提前暖機 GAS（與快照讀取並行），降低冷啟動導致 projects 超時風險
  try { warmGAS(); } catch (e) { /* intentionally ignored: optional fallback failure */ }
  let hasLocal = false;
  ...
  // 背景遷移舊 main
  migrateLegacyMain().catch(function(){ /* intentionally ignored: optional fallback failure */ });
  ...
  if(hasBypass_() && TreeSnapshot){ try{ await TreeSnapshot.delete(SNAP_PROJECTS); }catch(e){ /* intentionally ignored: optional fallback failure */ } if(preloadPid){ try{ await TreeSnapshot.delete(snapKeyForProject(preloadPid)); }catch(e){ /* intentionally ignored: optional fallback failure */ } } }
  ...
  try{
    ...
    if (TreeSnapshot && Array.isArray(state.PROJECTS) && state.PROJECTS.length) {
      (async function hydrateFast(){
        try{
          var jobs=state.PROJECTS.map(function(pp){ var pid=normalizePid(pp.project_id); if(!pid||state.treeSearchIndex.has(pid)) return null; return TreeSnapshot.load(snapKeyForProject(pid)).then(function(snap){ if(Array.isArray(snap)&&snap.length) applyTreesForProject(pid, snap, {saveSnapshot:false}); }).catch(function(){ /* intentionally ignored: optional fallback failure */ }); });
          await Promise.all(jobs.filter(Boolean));
          drawProjects();
        }catch(_){ drawProjects(); }
      })();
    } else { drawProjects(); }
    if (preloadPid) loadTreesForProject(preloadPid).catch(function(){ /* intentionally ignored: optional fallback failure */ });
    (async function warmAndReconcile(){
      try{
        ...
        if(Array.isArray(bt)) { var ch=reconcileFromBootstrap_(bt); try{ console.warn('[loader] warm counts pids='+(new Set(bt.map(function(x){return normalizePid(x.project_id);}))).size+' ch='+ch); }catch(e){ /* intentionally ignored: optional fallback failure */ } }
        if(!hasBypass_()){
          // 🔥 [Bugfix] 重試上限 8 次，防止 isLocating 永鎖時無限遞迴消耗 GAS quota
          (function scheduleRevalidate(attempt){ attempt = attempt || 0; var _d=1200; try{ _d=(typeof state!=='undefined' && state.isLocating)?2600:1200; }catch(e){ /* intentionally ignored: optional fallback failure */ }
            if(attempt > 8){ return; }
            setTimeout(async function(){ try{ if(typeof state!=='undefined' && state.isLocating){ scheduleRevalidate(attempt+1); return; } }catch(e){ /* intentionally ignored: optional fallback failure */ }
              try{ var bf=await ApiService.get('bootstrap', {nocache:'1'}); var btf=(bf&&bf.data&&bf.data.trees)?bf.data.trees:[]; _bootstrapFallbackCache={data:btf, ts:Date.now()}; var ch2=reconcileFromBootstrap_(btf, {forceEmptyApply:true}); try{ console.warn('[loader] revalidate changed='+ch2+' total='+btf.length); }catch(e){ /* intentionally ignored: optional fallback failure */ } }catch(e){ /* intentionally ignored: optional fallback failure */ }
            }, _d);
          })(0);
        }
    ...
    try{
      ...
      if (res && res.data && (res.data.projects || res.data.trees)){
        ...
        if (TreeSnapshot){
          TreeSnapshot.save(SNAP_PROJECTS, projects).catch(function(){ /* intentionally ignored: optional fallback failure */ });
          ...
          for(const [pid,arr] of byPid) TreeSnapshot.save(snapKeyForProject(pid), arr).catch(function(){ /* intentionally ignored: optional fallback failure */ });
        }
