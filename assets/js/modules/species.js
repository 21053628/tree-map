...
function _syncState(list,prom){ try{ state.speciesCache=list; state.speciesPromise=prom||null; }catch(e){ /* intentionally ignored: optional fallback failure */ } }
...
function snapshotLoad(){
  try{ if(TreeSnapshot&&TreeSnapshot.load) return TreeSnapshot.load(SNAP_KEY); }catch(e){ /* intentionally ignored: optional fallback failure */ }
  return Promise.resolve(null);
}
function snapshotSave(list){
  try{ if(TreeSnapshot&&TreeSnapshot.save) TreeSnapshot.save(SNAP_KEY,list).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }catch(e){ /* intentionally ignored: optional fallback failure */ }
}
...
export async function load(opts){
  ...
  _promise=(async function(){
    ...
    if(!force){
      try{ snapshotList=await snapshotLoad(); }catch(e){ /* intentionally ignored: optional fallback failure */ }
      if(Array.isArray(snapshotList)&&snapshotList.length){
        ...
        if(normSnap.length){
          ...
          (async function bg(){
            try{ var fresh=await fetchViaApi(); if(!fresh) fresh=await fetchStatic(); var norm=fresh.map(normalizeSpecies).filter(Boolean); _cache=norm; buildIndexes(_cache); _loadedAt=Date.now(); _syncState(_cache,null); snapshotSave(_cache); console.log('species bg refresh:'+_cache.length); }catch(e){ /* intentionally ignored: optional fallback failure */ }
          })();
          return _cache;
        }
      }
    ...
    var raw=null;
    try{ raw=await fetchViaApi(); }catch(e){ /* intentionally ignored: optional fallback failure */ }
...
export function clear(){
  ...
  try{
    if (TreeSnapshot) { if (TreeSnapshot.remove) TreeSnapshot.remove(SNAP_KEY).catch(function(){ /* intentionally ignored: optional fallback failure */ }); else if (TreeSnapshot.save) TreeSnapshot.save(SNAP_KEY,[]).catch(function(){ /* intentionally ignored: optional fallback failure */ }); }
    if (CacheManager.notifySwInvalidate) CacheManager.notifySwInvalidate('species');
    if (ApiService.clearCache) { try { ApiService.clearCache(); } catch (e2) { /* intentionally ignored: optional fallback failure */ } }
  }catch(e){ /* intentionally ignored: optional fallback failure */ }
}
