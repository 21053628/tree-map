...
import { AuditLog } from './audit-log.js';
    ...
    try {
      ...
      for (var i = 0; i < cssText.length; i++) {
        ...
        if (ch === '{') depth++;
          ...
          depth--;
          if (depth === 0) {
            try { sheet.insertRule(buffer, sheet.cssRules.length); } catch (e) { /* intentionally ignored: optional fallback failure */ }
            buffer = '';
          }
        }
      }
  ...
  function init() {
    ...
    // 其他分頁完成 queue/claim/sync 後即時刷新，不必等下一次 polling。
    window.addEventListener('treemap:syncchange', function (event) {
      if (!event || !event.detail || event.detail.source !== window.__treeMapSyncTabId) refresh();
    });
