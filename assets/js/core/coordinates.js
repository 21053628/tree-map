...
function getProjections(){ return PROJECTIONS; }
function initProj4(){
  const pj=getProj4(); if(!pj) return null; if(proj4Transform) return proj4Transform;
  try{ const projs=getProjections(); try{ const tr=pj(projs.WGS84, projs.HK80); if(tr&&typeof tr.forward==='function'){ proj4Transform=tr; return tr;}}catch(e){ /* intentionally ignored: optional fallback failure */ } proj4Transform=pj; return pj; }catch(e){ return null; }
}
...
function warnProj4Missing_(){
  ...
  _warnedProj4 = true;
  try { console.warn('[coordinates] proj4 尚未載入：同步轉換暫返回 null，已觸發非同步載入'); } catch(e){ /* intentionally ignored: optional fallback failure */ }
  try { ensureProj4().catch(function(){ /* intentionally ignored: optional fallback failure */ }); } catch(e){ /* intentionally ignored: optional fallback failure */ }
}
