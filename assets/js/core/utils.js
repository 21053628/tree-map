...
/**
 * 安全 HTML 清理：優先使用 DOMPurify；未載入時使用保守的原生 fallback。
 * fallback 會移除危險元素、事件屬性及非 http(s) URL，絕不直接回傳未清理 HTML。
 */
export function sanitizeHtml(html) {
  ...
  try {
    if (typeof document === 'undefined' || !document.createElement) return escapeHtml(value);
    const template = document.createElement('template');
    template.innerHTML = value;
    template.content.querySelectorAll('script,style,iframe,object,embed,base,form').forEach((el) => el.remove());
    template.content.querySelectorAll('*').forEach((el) => {
      Array.from(el.attributes).forEach((attr) => {
        ...
        if (name.startsWith('on') || name === 'srcdoc' || name === 'formaction') {
          ...
          && (!/^(?:https?:|mailto:|#|\/)/i.test(attrValue) || attrValue.startsWith('//'))) {
          el.removeAttribute(attr.name);
        }
      });
    });
